...
void TEA5767N::setFrequency(float _frequency) {
	...
	if (hiInjection) {
		setHighSideLOInjection();
		frequencyW = 4 * ((frequency * 1000000) + IFcounter*4096L) / 32768L;
	} else {
		setLowSideLOInjection();
		frequencyW = 4 * ((frequency * 1000000) - IFcounter*4096L) / 32768L;
	}
...
void TEA5767N::transmitData() {
	TWI_send(TEA5767_I2C_ADDRESS, transmission_data, 5);
	_delay_us(100);
}
...
void TEA5767N::readStatus() {
	TWI_receive(TEA5767_I2C_ADDRESS, reception_data, 5);
	_delay_us(100);
}
...
float TEA5767N::getFrequencyInMHz(unsigned int frequencyW) {
	if (hiInjection) {
		return (((frequencyW / 4.0) * 32768.0) - IFcounter*4096L) / 1000000.0;
	} else {
		return (((frequencyW / 4.0) * 32768.0) + IFcounter*4096L) / 1000000.0;
	}
}
...
uint8_t TEA5767N::getIFresult(){
	transmitData();
	_delay_ms(27);
	readStatus();
	return reception_data[THIRD_DATA]&0b01111111;
}
